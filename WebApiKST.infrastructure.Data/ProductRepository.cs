...
namespace WebApiKST.infrastructure.Data
{
    public class ProductRepository
    {
        ...
        public static string constr = "server=192.168.1.10;user=MiluskaDev;database=atahost_kststore;port=3306;password=miluska123";
        ...
        public ApiResponseDTO DeleteProduct(ProductoDTO objproducto)
        {
            ...
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(constr))
                {
                    using (MySqlCommand cmd = new MySqlCommand("SP_DELETE_PRODUCT", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.AddWithValue("@idproducto", objproducto.id_prod);
            ...
            {
                resp.pcode = 0;
                resp.pmessage = ex.Message.ToString(); 
            ...
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(constr))
                {
                    using (MySqlCommand cmd = new MySqlCommand("SP_UPDATE_PRODUCT", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@fch_lanzamiento", "2020-12-22");
                        cmd.Parameters["@fch_lanzamiento"].Direction = ParameterDirection.Input;

                        cmd.Parameters.AddWithValue("@valorizacion", _objRegister.valoracion);
                        cmd.Parameters["@valorizacion"].Direction = ParameterDirection.Input;

                        cmd.Parameters.AddWithValue("@idformato", _objRegister.idformato);
                        cmd.Parameters["@idformato"].Direction = ParameterDirection.Input;

                        cmd.Parameters.AddWithValue("@idcoleccion", _objRegister.id_coleccion);
